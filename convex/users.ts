...
export const ensureGuest = mutation({ args: { sessionId: v.string(), username: v.string() }, handler: async (ctx, args) => {
  ...
  const existing = await ctx.db.query("users").withIndex("by_session", (q) => q.eq("sessionId", args.sessionId)).first();
  const owners = await ctx.db.query("users").withIndex("by_name", (q) => q.eq("normalizedUsername", normalizedUsername)).collect();
  const owner = owners.find((candidate) => candidate.sessionId === args.sessionId) ?? owners[0];
